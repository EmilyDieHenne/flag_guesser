...
import { country_flags } from '@/assets/flags/country_flags'
...
const currentFlagImage = computed(() => {
    ...
    const fileName = country_flags[currentCountryId.value].countryCode
...
function getCountryNameById(id: number) {
    return country_flags[id].countryName
}

function rollRandomNumber() {
    const response = Math.floor(Math.random() * country_flags.length)
